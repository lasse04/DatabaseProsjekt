...
from formater_seter import sorter_seter
...
Stoppnummer beskriver at setene er ledige på flyvningen mellom stoppnummer og stoppnummer + 1.
"""
def finn_ledige_seter(flyruteNr: str, dato: str):
    cursor = get_cursor()
    
    billett_sql = """SELECT DISTINCT startStopp, sluttStopp, seteNr FROM Billett
    ...
    ledige_seter = {stopp: sorter_seter(seter) for stopp, seter in ledige_seter.items()}
    return ledige_seter
...
def finn_ledige_seter_tur(flyruteNr, dato, startStopp, sluttStopp):
    ...
    return sorter_seter(ledige_seter_tur)

if __name__ == "__main__":
    ledige_seter = finn_ledige_seter("WF1302", '2025-04-01')
    for stopp, seter in ledige_seter.items():
        print(f"{stopp}: {seter}")
